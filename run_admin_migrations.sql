-- Combined Admin Role Migrations
-- Run this in your Supabase SQL Editor

-- Part 1: Add admin role and comprehensive activity logging system
...
DROP TRIGGER IF EXISTS log_applicant_activity_trigger ON public.applicants;
...
DROP TRIGGER IF EXISTS log_message_activity_trigger ON public.messages;
...
DROP TRIGGER IF EXISTS log_interview_activity_trigger ON public.scheduled_interviews;
...
-- Part 2: Update the auth trigger to handle admin role
...
-- Part 3: Create an admin user (replace with your email)
...
BEGIN
  ...
  RAISE NOTICE 'To create an admin user, run: UPDATE users SET role = ''admin'' WHERE email = ''your-email@example.com'';';
END $$; 
...
export default function MessagesPage() {
  ...
  const fetchConversations = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        id,
        applicant:applicants(name, avatar_url),
        job:jobs(title),
    ...
    const transformedData: Conversation[] = (data || []).map((conv: any) => ({
      id: conv.id,
      applicant: {
        name: conv.applicant[0].name,
        avatar_url: conv.applicant[0].avatar_url,
      },
      job: {
        title: conv.job[0].title,
      },
            ...
            {conversations.map((conversation) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <Avatar>
                    <AvatarImage src={conversation.applicant.avatar_url || ''} />
                    <AvatarFallback>
                      {conversation.applicant.name.charAt(0)}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium truncate">{conversation.applicant.name}</p>
